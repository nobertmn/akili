  ...
  <div class="d-flex justify-content-center align-items-start h-100 pt-1">
    <div class="w-90">
      <div class="d-flex justify-content-between align-items-center">
        <h4 class="mb-1">Asset Transfers </h4>
      </div>
      <vue-good-table
        :columns="columns"
        :rows="rows"
          ...
          >
            <span
              ...
              v-if="
                ...
                (parseFloat(props.row.scheduled_tansfer_date_time)*1000) < timeNow
              "
            >
              Outdated
            </span>
    ...
    <SimpleModal
      :id="toBeEdited ? 'add-transfer-modal' : 'add-transfer-modal'"
      :title="toBeEdited ? 'Edit Assets Transfer Form' : 'Schedule New Asset Transfer'"
    ...
    <SimpleModal
      id="delete-asset-transfer-modal"
      title="Delete Asset Transfer Comfirmation"
    ...
    >
      <DeleteConcentForm
        ...
        :seed="{ transfer: [itemToDelete.id] }"
        :pst="pst"
        :title="'This will remove ' + (itemToDelete.title && itemToDelete.title.length > 0?itemToDelete.title:'the selected transfer')"
...
import AddTransferAssetsForm from "@/components/forms/AddTransferAssetsForm.vue";
...
export default {
  ...
  methods: {
    ...
    loadDependencies(callBack) {
      ...
      vm.fetch("transferAssetsDependencies", function (thisVm, res) {
        thisVm.rows = res.transfers;
        callBack();
      });
    },
    onClickAddBtn() {
