    ...
    >
      ...
      <div class="d-flex justify-content-between">
        ...
        <div>
          ...
          {{ viewItem.scheduled_tansfer_date }}
        </div>
        <div v-if="viewItem.status == 'issued'" class="ms-2">
          <span class="text-muted me-1">TRANSFER DATE:</span>
          {{ viewItem.transfer_date }}
        </div>
      </div>
      ...
      <h4 class="mt-1 border-bottom border-top pb-1 pt-1 mt-2 text-uppercase">
        ...
        <span v-if="viewContext == 'issue'">ISSUING -  </span>
        <span v-if="viewContext == 'receive'">RECEIVING -  </span>
        <span v-if="viewItem.title.length > 0">{{ viewItem.title }}</span>
        <span v-else>
          Moving Assets from {{ viewItem.fromBranch.name }} to
          {{ viewItem.toBranch.name }}
        </span>
      </h4>
      ...
      >
        <div v-if="activePageIndex == 0" style="min-height: 400px" class="py-1">
          <TransferDetailsViewer :view-item="viewItem" />
        </div>
        <div v-if="activePageIndex == 1" style="min-height: 400px" class="p-1">
            <vue-good-table
                :columns="transferAssetsColumns"
                :rows="transferAssets"
        ...
        <div v-if="activePageIndex == 2" style="min-height: 400px" class="py-1">
            <div class="d-flex flex-row justify-content-start flex-wrap">
                <div 
                  v-for="(supportDocument, docIndex) in transferSupportDocuments"
                  :key="docIndex"
...
import TransferDetailsViewer from "@/components/viewers/TransferDetailsViewer.vue";
...
export default {
  ...
  data() {
    return {
      ...
      viewItem: null,
      transferAssets: [],
      transferSupportDocuments: [],
      iconRefs: Utils.fileTypeIconRef(),
      transferAssetsColumns:[
        {
          label: "Asset Name",
          field: "asset.name",
        },
  ...
  methods: {
    ...
    loadDependencies() {
      let vm = this;
      vm.fetchItem("transferDetails", vm.viewItem, function (thisVm, res) {
        thisVm.viewItem = res.transfer;
        thisVm.transferAssets = res.transferAssets;
        thisVm.transferSupportDocuments = res.transferSupportDocuments;
        //selected assets tab title count
        var numAsets = thisVm.transferAssets.length;
        thisVm.pages[1].title += numAsets>0?` (${numAsets})`:'';
        //supporting documents tab title count
        var num = thisVm.transferSupportDocuments.length;
