<template>
    <MDBCard text="center" class="align-self-stretch " :class="{'w-90': !isTabsMode, 'mb-0': isTabsMode, 'verticle-tabs': isVerticle}" :shadow="isTabsMode?'0':''" >
        <MDBProgress class="m-1" :height="1" v-if="!isTabsMode">
            <MDBProgressBar :value="progressPercentage" bg="success"  />
        </MDBProgress>
        <MDBCardHeader  :class="isTabsMode?'p-0 tabs-stepper-header ':'pt-0 pb-1'" >
            <ul class="nav nav-tabs card-header-tabs" :class="isTabsMode?'w-100 border-bottom':''">
                <li class="nav-item" v-for="(page, index) in pages" :key="index">
                    <a class="nav-link" 
                        :class="{active: innerActivePageIndex == index, finised: innerActivePageIndex > index && !isTabsMode}" 
                        ...
                        <span class="onboarding-step-no" v-if="!isTabsMode">{{page.number}}</span>
                        {{page.title}}
                    </a>
                </li>
                <!--
        ...
        <MDBCardBody :class="isTabsMode?'ps-1 pe-1 tabs-stepper-body':''">
            <slot></slot>
        </MDBCardBody>
        <MDBCardFooter class="d-flex justify-content-between" v-if="!isTabsMode">
            <div>
